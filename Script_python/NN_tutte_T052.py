...
conf_T052_NT = np.load('../../T052_P109_NT/configurazione_iniziale_completa_T052_P109_NT.npy')
...
msd_T052_NT = np.load('../../T052_P109_NT/msd_completo_T052_P109_NT.npy')
...
conf_T052_NT_train, conf_T052_NT_test, msd_T052_NT_train, msd_T052_NT_test = train_test_split(conf_T052_NT, msd_T052_NT, shuffle = True, train_size = 0.8, random_state = 7890)
...
class Dataset(object): #Is object really useful?
    def __init__(self, partition = 'train', num_points = 4096):  
        ...
        elif partition == 'T052_NT_test':
           self.data, self.label = conf_T052_NT_test, msd_T052_NT_test # Test
...
test_T052_NT = Dataset(partition = 'T052_NT_test', num_points = 4096)
...
model.load_weights("model_checkpoints/DGCNN_k20_e30_bs16_cpMax.h5")
#model.load_weights("model_checkpoints/prova.h5")

test_loss, test_mae = model.evaluate(test_T052_NT.X, test_T052_NT.y, verbose = 1) #Restituisce prima la loss e poi le metrics definite in model.compile
...
predictions_T052_NT = model.predict(test_T052_NT.X, verbose = 1) #prediction vs test.y, istogramma di test.y - prediction, predicted value for new unlabeled data (or pretend that you do not have a label)
...
np.save('predictions_k20_e30_bs16_cpMax_T052.npy', predictions_T052_NT)
np.save('test_y_k20_e30_bs_16_cpMax_T052.npy', test_T052_NT.y)
